	...
	public String upload(Usuario usuario) throws IOException {
	 ...
	 public String efetuaLogin(Usuario usuario, HttpSession session) {
		 if(new UsuarioDAO().existeUsuario(usuario)) {
			 usuario = new UsuarioDAO().getUser(usuario.getLogin());
			 session.setAttribute("usuarioLogadoNome", usuario.getLogin());
	 ...
	 public String lista(Usuario usuario, HttpSession session, ModelMap model) {
		 TarefasDAO tarefasDao = new TarefasDAO();
		 session.setAttribute("listaTarefas", tarefasDao.getListaId((Integer)session.getAttribute("usuarioLogadoId")));
